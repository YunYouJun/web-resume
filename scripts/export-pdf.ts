...
Usage:
  pnpm export:pdf -- --input <path-or-url> [--output resume.pdf] [--scale 1]
...
export async function loadResume(input: string) {
  ...
  if (/^https?:\/\//i.test(input)) {
    ...
    text = await readFile(resolve(process.cwd(), input), 'utf8')
  }

  const schemaPath = resolve(import.meta.dirname, '../src/assets/schema/resume.schema.json')
  ...
  if (!validate(resume)) {
    ...
    throw new Error(`Resume YAML does not match the schema: ${details}`)
  }
...
export async function exportPdf(options: ExportOptions) {
  ...
  try {
    ...
    const resumeUrl = `${url}/?url=${encodeURIComponent('/__resume.yml')}&mode=preview`
