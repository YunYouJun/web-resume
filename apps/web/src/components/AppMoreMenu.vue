...
const { commands: allCommands, execute, find } = useResumeCommands()
const { t } = useI18n()
  ...
  <DropdownMenuRoot>
    ...
    <DropdownMenuPortal>
      <DropdownMenuContent class="app-menu-content" :side-offset="8" align="end">
        <DropdownMenuLabel class="app-menu-label">
          {{ t('command.templates') }}
        </DropdownMenuLabel>
        <DropdownMenuRadioGroup :model-value="templateCommands.find(command => command.checked)?.id">
          <DropdownMenuRadioItem
