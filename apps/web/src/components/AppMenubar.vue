    ...
    <MenubarMenu>
      ...
      <MenubarPortal>
        <MenubarContent class="app-menu-content" :side-offset="8" align="start">
          ...
          <template v-if="settingsCommand">
            <MenubarSeparator class="app-menu-separator" />
            <MenubarItem
              class="app-menu-item"
              @select="select(settingsCommand)"
            >
              <div :class="settingsCommand.icon" aria-hidden="true" />
              <span>{{ settingsCommand.label }}</span>
              <kbd class="app-menu-shortcut">{{ settingsCommand.shortcut }}</kbd>
            </MenubarItem>
          </template>
        </MenubarContent>
      </MenubarPortal>
    ...
    <MenubarMenu>
      ...
      <MenubarPortal>
        <MenubarContent class="app-menu-content" :side-offset="8" align="start">
          ...
          <MenubarRadioGroup :model-value="templateCommands.find(command => command.checked)?.id">
            <MenubarRadioItem
              v-for="command in templateCommands"
              :key="command.id"
              class="app-menu-item"
              :value="command.id"
              @select="select(command)"
