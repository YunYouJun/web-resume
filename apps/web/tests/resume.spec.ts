...
test('reviews, converts, and undoes a legacy resume migration', async ({ page }) => {
  ...
  await expect(dialog.getByRole('button', { name: '转换并写回编辑器' })).toBeEnabled()
  await dialog.getByRole('button', { name: '转换并写回编辑器' }).focus()
  await page.keyboard.press('Tab')
  await expect.poll(() => page.evaluate(() => Boolean(document.activeElement?.closest('[role="alertdialog"]')))).toBe(true)
  await page.keyboard.press('Escape')
  await expect(dialog).toBeHidden()
  await expect(page.getByRole('button', { name: '检查并转换' })).toBeFocused()
  await page.getByRole('button', { name: '检查并转换' }).click()
  ...
  // Monaco detects macOS from the emulated user agent, not the host platform.
  const undoShortcut = await page.evaluate(() => /Macintosh|Mac OS X/.test(navigator.userAgent) ? 'Meta+Z' : 'Control+Z')
