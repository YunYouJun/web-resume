...
- 主域实际由 Cloudflare Worker `web-resume` 转发静态资源到 EdgeOne Pages 的 `web-resume.yunyoujun.cn`。使用仓库 `workers/wrangler.toml` 发布 Worker；它直接处理 `/api/*`，保留 `resume.yunle.fun` 的 Origin，并使用 `YLF_LOGIN_API_ENABLED=true`、`YLF_CLOUD_API_ENABLED=false`。
