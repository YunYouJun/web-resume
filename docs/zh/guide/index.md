...
在 **设置 → 简历默认值** 中可组合三种模板与以下样式：

- **配色**：默认经典黑白保持黑色标题和蓝色链接，与原版简历风格一致；也可选择森林绿、梅子紫、石墨灰。
- **字体**：现代无衬线、典雅衬线，使用设备本地字体，不下载额外字体文件。
- **内容间距**：舒展或紧凑，调整段落与章节间距。
- **照片形状**：方角、圆角或圆形，照片居中裁剪；地址仍填写在 `basics.image`（旧版为 `basics.avatar`）。

下方会即时预览当前简历；尚未加载简历时使用示例内容，不会改写你的草稿。点击 **重置配色与样式** 可恢复默认样式，所选模板不变。

外观偏好保存在本机，不改写 YAML。纯净预览、两种分享链接和独立 HTML 都保留配色、字体、间距与照片形状；浏览器打印/PDF 也使用相同样式。打开分享链接不会覆盖接收方的外观偏好。旧链接未指定外观时使用默认样式，非法参数也会回退到默认值。

字体使用本机回退字体，因此不同设备的字形及分页可能略有差异。发布前请检查导出效果。

### 分享预览

选择 **文件 → 复制分享链接**。链接包含源文件 URL 或内置示例 ID、所选模板和预览模式，不包含未保存的编辑器文本，也不包含浏览器本地个人信息。如需通过此方式分享修改结果，请先发布更新后的 YAML 并加载该地址，或使用下方的“复制当前内容链接”。

### 分享当前编辑内容

选择 **文件 → 复制当前内容链接**，即可分享当前 YAML，无需先上传文件。链接保留所选模板，接收方直接进入纯净预览，不会覆盖接收方的本机草稿。内容保存在 URL 的 `#resume=` fragment 中，不会随 HTTP 请求发送给服务器；它不是加密，任何获得完整链接的人都能读取简历。

此方式不包含本机个人信息覆盖，YAML 最大支持 48 KiB。聊天软件可能截断较长链接，较大简历建议使用源文件链接分享。

### 导出独立 HTML 并部署 GitHub Pages

在首页或编辑器选择 **文件 → 导出独立 HTML**，下载当前显示的简历（包含已启用的个人信息覆盖）和模板样式。导出文件不包含编辑器、工具栏和应用脚本，可直接在浏览器打开。

1. 将下载文件重命名为 `index.html`，放入用于发布的仓库。
2. 在 GitHub 仓库 **Settings → Pages** 中选择发布分支和目录。
3. 发布后访问 Pages 地址即可查看简历，无需部署整个 Web Resume 应用。

远程照片、图片和字体仍使用原地址；导出不会把外部资源打包到文件中。请确保这些地址长期可访问，部署前检查联系信息和图片。

### 照片和模板

标准 JSON Resume 在 `basics.image` 中填写照片地址；旧版格式使用 `basics.avatar`。可以使用 HTTPS 地址或站点内的图片路径，例如：
...
目前提供清晰经典、紧凑单页、专业侧栏三种模板，可在 **视图 → 模板** 或模板库切换。照片在三种模板中均可显示。设置页的深浅色用于应用界面，独立 HTML 默认采用适合打印的浅色页面。
